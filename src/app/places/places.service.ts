...
export class PlacesService {
  ...
  loadUserPlaces() {
    return this.fetchPlaces('http://localhost:3000/user-places', 'Failed to fetch your places. Please try again later.').pipe(tap({
      next: (places) => {
  ...
  removeUserPlace(place: Place) { 
    ...
    return this.httpClient.delete(`http://localhost:3000/user-places/`+ place.id).pipe(
      catchError((error) => {
        this.userPlaces.set(prevPlaces);
        this.errorService.showError("Failed to remove place from your places. Please try again later.");
        return throwError(() => new Error("Failed to remove place from your places. Please try again later."));
      })
    );
  }
